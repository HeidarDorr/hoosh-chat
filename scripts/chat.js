...
function gptComment() {
    ...
    document.querySelectorAll('.gpt-comment')[document.querySelectorAll('.gpt-comment').length - 1].insertAdjacentElement('afterbegin', pTag)
    ...
    // const boldWords = chatgptResponse.match(/`([^`]+)`/g)
    // if (boldWords) {
    //     boldWords.forEach(match => {
    //         const strongTag = document.createElement('code')
    //         strongTag.innerText = match

    //         chatgptResponse = chatgptResponse.replace(match, strongTag)
    //     })
    // }

    if (!chatgptResponse.match(/```[^`]*```([\s\S]*?)(?=\s|$)/)) {
        modifiedText = chatgptResponse
        words = modifiedText.split(' ')

        typeAnimation(pTag, waitingCircle, words)
    }
    else { // if there is a code zone in response
        codeZoneHandler(pTag, waitingCircle)
    }
}



/////////////////////////////// (development code) zone
let modifiedText

function codeZoneHandler(pTag, waitingCircle) {
    ...
    preTag.innerHTML = `
        <div class="bg-black rounded-md flex flex-col overflow-hidden">
            <div class="flex items-center relative text-gray-200 bg-gray-800 px-4 py-2 text-xs font-sans justify-between rounded-t-md">
                ...
                <button class="copy-code-btn flex ml-auto gap-2 items-center">
                ...
                <span style="display: none;" class="copied relative">Copied!</span>
            </div>
            <code class="hljs !px-3">${codeZone3}</code>
        </div>
        `
    const parts = chatgptResponse.split(codeZone)
    console.log(parts[0] + codeZone + parts[1])
    ...
    pTag.parentElement.append(preTag)

    const pTag2 = document.createElement('p')
    pTag.parentElement.append(pTag2)
    modifiedText = parts[1]
    const words2 = modifiedText.split(' ')
    typeAnimation(pTag2, waitingCircle, words2)
}
...
function typeAnimation(pTag, waitingCircle, words) {
    ...
    function displayText() {
        ...
        if (currentIndex < words.length) {
            pTag.innerText += words[currentIndex] + ' '
            currentIndex++
